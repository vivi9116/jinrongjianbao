...
def beijing_timestamp(now_utc: str | None = None) -> str:
    parsed = (
        dt.datetime.fromisoformat(now_utc.replace("Z", "+00:00"))
        if now_utc
        else dt.datetime.now(dt.timezone.utc)
    )
    return parsed.astimezone(dt.timezone(dt.timedelta(hours=8))).strftime("%Y-%m-%d %H:%M BJT")
...
def fetch_market_quotes(session: Any | None = None) -> list[dict[str, str]]:
    ...
    for label, symbol in YAHOO_SYMBOLS.items():
        ...
        if price is None:
            continue
        seen = "timestamp unavailable"
        if timestamp:
            seen = dt.datetime.fromtimestamp(timestamp, dt.timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
        rows.append(
            {
                "label": label,
                "price": f"{price:,.2f}",
...
def format_market_quotes(rows: list[dict[str, str]]) -> str:
    if not rows:
        return "- \u5b9e\u65f6\u62a5\u4ef7\u6682\u4e0d\u53ef\u7528\uff1b\u8bf7\u6838\u5bf9\u4ea4\u6613\u6240\u3001\u5238\u5546\u7ec8\u7aef\u6216 Yahoo Finance quote feed\u3002"
    return "\n".join(
...
def major_move_reasons(rows: list[dict[str, str]]) -> list[str]:
    ...
    for row in rows:
        ...
        threshold = thresholds.get(row.get("label", ""))
        if change is not None and threshold is not None and abs(change) >= threshold:
            reasons.append(f"{row['label']} {row['change']} exceeds {threshold:.1f}% threshold")
    return reasons
...
- \u4e8b\u5b9e\u4e0e\u4ef7\u683c\u6765\u81ea\u516c\u5f00\u6570\u636e\u6e90\uff1b\u65b9\u5411\u5224\u65ad\u9700\u8981\u7ed3\u5408\u4ed3\u4f4d\u3001\u671f\u9650\u548c\u98ce\u9669\u9884\u7b97\uff0c\u4e0d\u6784\u6210\u6295\u8d44\u5efa\u8bae\u3002

\u5e02\u573a\u4ef7\u683c
{format_market_quotes(quote_rows)}{quote_warning}

\u9a71\u52a8\u56e0\u7d20
- \u5229\u7387: \u6838\u5bf9 Federal Reserve \u6700\u65b0\u58f0\u660e\u3001CME FedWatch \u5229\u7387\u8def\u5f84\u548c\u7f8e\u503a\u62cd\u5356\u7ed3\u679c\u3002
- \u901a\u80c0: \u6838\u5bf9 BLS CPI/PPI \u65f6\u95f4\u548c\u5206\u9879\uff0c\u80fd\u6e90\u3001\u4f4f\u623f\u3001\u6838\u5fc3\u670d\u52a1\u662f\u5173\u952e\u3002
- \u80fd\u6e90: \u6838\u5bf9 EIA \u5468\u5ea6\u5e93\u5b58\u3001WTI/Brent \u671f\u9650\u7ed3\u6784\u548c\u5730\u7f18\u653f\u6cbb\u6d88\u606f\u3002
- \u98ce\u9669\u504f\u597d: \u540c\u65f6\u89c2\u5bdf VIX\u3001\u7f8e\u5143\u3001\u6bd4\u7279\u5e01\u3001\u534a\u5bfc\u4f53\u548c\u4e9a\u6d32\u4e3b\u8981\u6307\u6570\u3002

\u98ce\u9669\u6e05\u5355
- CPI\u3001\u5c31\u4e1a\u3001\u96f6\u552e\u9500\u552e\u3001PMI\u6216\u592e\u884c\u8bb2\u8bdd\u663e\u8457\u504f\u79bb\u9884\u671f\u3002
...
def send_telegram_message(token: str, chat_id: str, text: str, session: Any | None = None) -> dict[str, Any]:
    client = session or requests
...
def run(mode: str, now_utc: str | None = None, session: Any | None = None) -> dict[str, Any]:
    ...
    if mode == "alert":
        try:
            ...
            return {"ok": True, "skipped": True, "reason": f"quote feed unavailable: {type(exc).__name__}"}
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=sorted(MODE_TITLES), default="morning", help="Brief mode to generate.")
    ...
    try:
        ...
        if args.mode != "alert":
            raise
        result = {"ok": True, "skipped": True, "reason": f"quote feed unavailable: {type(exc).__name__}"}
    print(f"telegram_ok={result.get('ok')}")
    if result.get("skipped"):
        print(f"skipped=True reason={result.get('reason')}")
